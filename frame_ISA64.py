...
def images_resize_aspectratio_v2(arImages: np.array, nMinDim:int = 256) -> np.array:
    #print(nMinDim)
    nImages, _, _, _ = arImages.shape
    liImages = []
    for i in range(nImages):
        arImage = image_resize_aspectratio(arImages[i, ...], nMinDim)
        #print(arImage.shape)
        liImages.append(arImage)
    return np.array(liImages)

def images_resize_aspectratio_v3(arImages: np.array, nMinDim:int = 256) -> np.array:
    #print(nMinDim)
    nImages, _, _, _ = arImages.shape
    liImages = []
    print('Hello')
    for i in range(nImages):
        #print(arImages[i, ...].shape)
        arImage = cv2.resize(arImages[i, ...], dsize = (nMinDim,nMinDim))
        #print(arImage.shape)
        liImages.append(arImage)
    return np.array(liImages)

def images_convertToGray(arImages: np.array, nMinDim:int = 256) -> np.array:
    #print(nMinDim)
    nImages, _, _, _ = arImages.shape
    liImages = []
    for i in range(nImages):
        arImage = cv2.cvtColor(arImages[i, ...], cv2.COLOR_BGR2GRAY)
        #print(arImage.shape)
        liImages.append(arImage)
    return np.array(liImages)
...
def frames2files(arFrames:np.array, sTargetDir:str):
    ...
    for nFrame in range(arFrames.shape[0]):
        cv2.imwrite(sTargetDir + "/frame%04d.png" % nFrame, arFrames[nFrame, :, :, :])
    return


def files2frames(sPath:str, convertToGrapy:bool=False, resizeImg:bool=False) -> np.array:
...
def images_crop(arFrames:np.array, nHeightTarget, nWidthTarget) -> np.array:
    ...
    if (nHeight < nHeightTarget) or (nWidth < nWidthTarget):
        #Hamzah
        ##############################arFrames = images_resize_aspectratio_v2(arFrames, nHeightTarget)
        arFrames = images_resize_aspectratio_v3(arFrames, nHeightTarget)
        #print("Hi")
...
def images_normalize_withGrayscale(arFrames:np.array, nFrames:int, nHeight:int, nWidth:int, bRescale:bool = True) -> np.array(float):
    """ Several image normalizations/preprocessing: 
        - downsample number of frames
        - crop to centered image
        - rescale rgb 0-255 value to [-1.0, 1.0] - only if bRescale == True
    ...
    arFrames = images_convertToGray(arFrames, nHeight, nWidth)
...
if __name__ == '__main__':
    ...
    unittest("/media/eye/B29A2B4F9A2B0F81/Hamzah/ArabSign/Final/video/01/train", 1000)
    #unittest("data-set/04-chalearn/010/train", 1000)